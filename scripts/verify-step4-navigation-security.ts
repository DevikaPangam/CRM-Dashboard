import { TAB_MODULE_ACCESS_MAP, getAuthorizedDefaultTab } from '../src/constants/moduleAccess';

// Baseline permissions for all 7 production roles (matching public.role_permissions schema)
const BASELINE_PERMISSIONS: Record<string, Record<string, string[]>> = {
  super_admin: {
...
function createRoleCanResolver(role: string) {
  const rolePerms = BASELINE_PERMISSIONS[role] || {};
  return (moduleKey: any, action: any) => {
